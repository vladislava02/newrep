...
class WorkWithString {
        ...
        while (true) {
            ...
            if (choice.compareTo("1") == 0) {//сравнение выбранного пользователем пункта меню
                ...
                if (choice2.compareTo("1") == 0) {
                    ...
                    addRandom();//вызов метода добавления текста в указанную позицию в файле
                }
    ...
    public void textFromfile() throws IOException {//метод чтения текста из файла и определение количества гласных, пробелов и общего количества букв
        System.out.println("Введите имя файла:");
        filename = in.readLine();//ввод имени файла, которое следует вводить учитывая расширение
        ...
        for (int i = 0; i < arrayWords.length-1; i++) {
            if(Character.toLowerCase(arrayWords[i].charAt(arrayWords[i].length()-1)) == Character.toLowerCase(arrayWords[i+1].charAt(0))) {
                System.out.println("Слова у которых последняя буква = первой букве: "+ arrayWords[i] + " " + arrayWords[i+1]);
            }
        }
